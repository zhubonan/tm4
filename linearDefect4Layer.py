# -*- coding: utf-8 -*-
"""
Created on Thu Jun  2 19:05:19 2016
Based on LineDefect3Layer but use four layers instead
@author: Bonan
"""
...
class CrossSection3L(CrossSection):
    ...
    def alignHelices(self, pointIndex):
        ...
        s[2].phyParas['aor'] = 2 * pi * s[1].phyParas['t'] / \
        s[1].phyParas['p'] + s[1].phyParas['aor'] + 5 *  pi * pointIndex / len(self.t)
        
        s[3].phyParas['aor'] = 2 *pi * s[0].phyParas['t'] / \
...
if __name__ == '__main__':
    ...
    for pitches in pitchesList1:
        ...
        h4 = heli(CNC, pitches[3], 1000)
        tmp = [h1,h2,h3,h4]
        #%% Set layer structure
        c = CrossSection3L(s, 5000,1000,4)
        c.setInterfaceFunction(f1,0)
        c.setInterfaceFunction(f2,1)
        c.setInterfaceFunction(f3,2)
