...
heli = sc.HeliCoidalStructure(testMaterial, p, h)
s.setStructure([heli])
bRes = s.scanSpectrum(wlRange,1)[1]
#%% Yeh's Method
l = scOld.H_Layers(scOld.Uniaxial_Material(ne,no),p,30,h)
yRes = []
for wl in wlRange:
...
tRes = abs((w**2+1)*(1-exp(-2j*k0*n2*h)) \
           / (2*w*(1+exp(-2j*k0*n2*h)) \
           - 1j*(w**2-1)*(1-exp(-2j*k0*n2*h))))**2

#%% Plotting
pl.plot(wlRange, bRes, label = "Berreman's")
pl.plot(wlRange, tRes, label = "Theotical")
pl.plot(wlRange, yRes, label = "Teh's")
pl.legend()