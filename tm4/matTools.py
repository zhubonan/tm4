...
class specData():
    ...
    def __init__(self, wl, spec, desc=None, shape=None):
        ...
        spec: an 1xN array or NxM array. Where N is the number of
        wavelengths and M is the number of such spectrum
    ...
    def _FTSpectrum(wl, spec, ns, paddling, k='linear'):
        ...
        be in nm. Can use this to see the effect of thin film interference
        and therby find the thickness of the film. The x axis is n*d(p)
        ...
        fspec = fft(paddled)
        n = paddled.size
        # We want to scale the x axis such it is labed as nd.
        # Note the ffted frequency
    ...
    def plot(self, indices, showPeaks=False, peakWidthRange=(10, 30), **argv):
        ...
        for i in range(n):
            ...
            if showPeaks:
                peakId = find_peaks_cwt(
                    spec[:, i], np.arange(*peakWidthRange), **argv)
    ...
    def getFilteredSpectrum(self, window_length=5, polyorder=2, **args):
        ...
        for i in range(n):
            r[:, i] = savgol_filter(
                self.spec[:, i], window_length, polyorder, **args)
    ...
    def getResampledSpectrum(self, ns=1000, k='linear'):
        ...
        for i in range(1, self.n):
            ...
            thisResampled = self._resampledSpectrum(
                1 / self.wl, thisSpec, ns, k)[1]
    ...
    def getRGBArray(self, scale=False):
        ...
        if scale is not True:
            ...
            for i in range(n):
                RGB[i, :] = specToRGB(
                    [self.wl, self.spec[:, i]], scale_factor=f[i])
        return RGB
    ...
    def get2DColouredImage(self, shape='auto', show=False, scale=False):
        ...
        if shape == 'auto':
            if self.spacialShape is not None:
    ...
    def get2DGreyScaleImage(self, show=False):
        ...
        if show:
            ...
            pl.colorbar()
            wlr = "{0:.0f}".format(
                self.range[0]) + " to " + "{0:.0f}".format(self.range[1])
    ...
    def plotLineSlice(self, start, finish, num, show=1):
        ...
        This uses scipy.ndimage.map_coordinates function.
        The spectral data is treated as a stack of 2D images along
        the first axes.
        start, finish: tuple of length 2s for starting and finishing
        coorindates
        ...
        """
        n = self.spec.shape[0]
        if type(self.spacialShape) is not None:
            l = self.spacialShape
        else:
            raise RuntimeError('Cannot get the shape of the spectrum')
        # use default range 425 to 800
        data = self.spec.reshape((n, l[0], l[1]))
        # Generate the coorindates
        x, y = np.linspace(start[0], finish[0], num), np.linspace(
            start[1], finish[1], num)
        # Preallocate storage space
        zi = np.empty((n, num))
        for i in range(n):
            # We interoplate at each wavelength
            # Need to transpose each images since the ordering is different
            # as in imshow the 1st axis is the Y axis not X
            zi[i] = sp.ndimage.map_coordinates(data[i].T, np.vstack((x, y)))
        if show:
            pl.imshow(zi, aspect='auto', extent=[
                      0, num, self.range[1], self.range[0]])
...
class scanData:
    ...
    def __init__(self, filename):
        ...
        # This is preffered as it let data access easier since we don't
        # care about saving
    ...
    def getCurrentSpecData(self, index=None):
        ...
        return specData(self.wl,
                        self.currentStruct.spec,
                        self.currentStruct.desc)
    ...
    def getCombinedSpecData(self, indices='all'):
        ...
        descList = map(lambda x: self._convertDescField(
            self.scan[x, 0].desc), indices)
