...
class OptSystem():
    """
    A class that represent a full set up. Contains multiple structures and
    front/back half-space. Optical properties should be extracted from
    a full system
    """
    # Attributes
    ...
    def setIncidence(self, wl, Theta=0, Phi=0):
        ...
        self.wl = wl
        # Calculate Kx from the property of front half-space,
        # Kx is then conserved
    ...
    def getTransferMatrix(self):
        ...
        frontinv = self.frontHalfSpace.getTransitionMatrix(self.Kx,
                                                           self.wl,
                                                           inv=True)
    ...
    def setThickness(self, tList):
        ...
        tList:
            a list containing the thicknesses of the structures in structures
        """
        if type(tList) == int:
            tList = [tList]
    ...
    def scanSpectrum(self, wlList,  coreNum=1, giveInfo=False, coupling='LL'):
        ...
        useProp: if set to True the result will be a list of OptProperties.
        Userful if want to use the full information from calculation.
        e.g. Check the effect
        of using analyser
        """
        result = []
